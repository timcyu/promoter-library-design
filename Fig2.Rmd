...
ggplot(induce_combo, aes(normalized_RNA_exp_Induced_12, ratio/normalized_RNA_exp_UnInduced_12)) + geom_point(aes(color=Proximal), size = 3, alpha = .6) + scale_color_manual(values = pal(10), breaks = legend_ord) +
  geom_smooth(method='gam', color = 'brown4') + annotation_logticks() + scale_x_log10(breaks = c(1,10,100)) + 
  scale_y_log10(breaks = c(1,10,100)) + xlab('Induced expression') + ylab('Induction Ratio/Basal Expression') + labs(title = 'Induction Ratio/Basal Expression vs. Induced Expression') + facet_wrap(~Proximal, scales = 'fixed') 
ggsave('Fig_Supp_1.pdf', width = 10, height = 8)
...
ggplot(induce_combo, aes(normalized_RNA_exp_UnInduced_12, ratio/normalized_RNA_exp_UnInduced_12)) + geom_point(aes(color=Proximal), size = 3, alpha = .6) + scale_color_manual(values = pal(10), breaks = legend_ord) +
  geom_smooth(method='gam', color = 'brown4') + annotation_logticks() + scale_x_log10(breaks = c(1,10,100)) + 
  scale_y_log10(breaks = c(1,10,100)) + xlab('UnInduced expression') + ylab('Induction ratio/Basal Expression') + labs(title = 'Induction Ratio/Basal Expression vs. UnInduced Expression') + facet_wrap(~Proximal, scales = 'fixed')

ggsave('Fig_Supp_2.pdf', width = 10, height = 8)
