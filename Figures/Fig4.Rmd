...
mu$Proximal <- with(mu, reorder(Proximal, induction, median))
mu$Distal_left <- with(mu, reorder(Distal_left, induction, median))
mu$Distal_right <- with(mu, reorder(Distal_right, induction, median))


scram_prox_avg <- median(filter(mu, Proximal == 'lacOscram')$induction)
#induce_multiple$Proximal <- with(induce_multiple, reorder(Proximal, induction, median))

# PROXIMAL
a <- ggplot(mu, aes(x = Proximal, y=induction/scram_prox_avg)) +
  ...
  labs(title = 'Distribution of Induction:Noise \n by Proximal Operator', 
       y = 'Induction Ratio:Noise', x = 'Proximal Operator', size = 30) +
