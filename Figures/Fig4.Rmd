...
combo_multiple_comp %>% 
  ...
  mutate(mean_diff = mean(ratio.y/ratio.x)) %>%
  ungroup() %>%
ggplot(., aes(x = Distal_left, y = Distal_right)) + geom_tile(aes(fill = mean_diff)) + theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 15), axis.text.y = element_text(size = 15)) +
  scale_fill_viridis(name = expression(frac('Multiple Induction', 'Combo Induction'))) + scale_x_discrete(limits = c("lacOscram", "lacO3", 'lacO2', 'lacO1', 'lacOsym')) + scale_y_discrete(limits = c("lacOscram", "lacO3", 'lacO2', 'lacO1', 'lacOsym')) + labs(title = 'Combinatorial Induction of Left/Right Distal Sites with Consensus -10/-35 and Proximal Osym/O1', x = 'Left Distal', y = 'Right Distal', size = 20) +
  theme(panel.background = element_rect(fill = "gray40")) + 
  geom_text(aes(label=signif(mean_diff, 3)), size=5)
  ggsave('Fig_4B.pdf', width = 12, height = 8)
