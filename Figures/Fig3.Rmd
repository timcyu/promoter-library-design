...
annotation_logticks() + scale_x_log10(breaks = c(0,1,10,100)) + scale_y_log10(breaks = c(0,1,10,100)) + xlab('Basal Expression') + ylab('Induced Expression') + labs(title = 'Induced Expression vs Basal Expression') + geom_abline(intercept = 0, slope = c(1,2,4,8), color = 'black', linetype = 'dashed', size = 0.75) + geom_text(aes(x = 1.5, y = 120, label = '8x')) + geom_text(aes(x = 2.7, y = 120, label = '4x')) + geom_text(aes(x = 8.7, y = 120, label = '2x')) + geom_text(aes(x = 91, y = 120, label = '1x'))
ggsave('Fig_3B.pdf', width = 10, height = 8)
...
a2 <- ggplot(basal_exp, aes(x = Library, y = ratio)) +
  ...
  scale_y_log10() +
  annotation_logticks(sides = 'l') + 
  ylab('Induction Ratio') + xlab('Promoter Architecture') + labs(title = 'Variation in induction ratio levels \n across different promoter architectures') + theme(axis.text.x = element_text(angle = 45, hjust = 1, size =10))

plot_grid(a1, a2)
ggsave('Fig_4B.pdf', width = 14, height = 8)
```
...
gg3 <- steric_box %>%
  ...
  geom_bar(position = "dodge", stat = 'identity') + scale_x_discrete(limits = c('no_up_ext_UV5', 'up_326x_ext_UV5', 'no_up_ext_10', 'up_326x_ext_10')) +   scale_fill_manual(values = pal4) + labs(y = 'Induced Expression', x = 'Regulatory element combination') + theme(axis.text.x = element_text(angle = 45, hjust = 1, size =10))                                                        
